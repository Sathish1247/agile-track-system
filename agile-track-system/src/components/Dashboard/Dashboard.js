    ...
    const handleGetDetails = async(scrumId) => {
        try {
            const response = await axios.get(
                `http://localhost:4000/scrums/${scrumId}`
            );
    ...
    return ( <
            ...
            user ? .role === "admin" && ( <
                ...
                showForm && ( <
                    ...
                    value = { newScrumName }
                    onChange = {
                        (e) => setNewScrumName(e.target.value)
                    }
                    required /
                    >
                    <
                    ...
                    value = { newTaskTitle }
                    onChange = {
                        (e) => setNewTaskTitle(e.target.value)
                    }
                    required /
                    >
                    <
                    ...
                    onChange = {
                        (e) => setNewTaskDescription(e.target.value)
                    }
                    required /
                    >
                    <
                    ...
                    onChange = {
                        (e) => setNewTaskStatus(e.target.value)
                    }
                    required >
                    <
                    ...
                    select value = { newTaskAssignedTo }
                    onChange = {
                        (e) => setNewTaskAssignedTo(e.target.value)
                    }
                    required >
                    <
...
ul > { " " } {
        scrums.map((scrum) => ( <
            li key = { scrum.id } > { " " } { scrum.name } { " " } <
            button onClick = {
                () => handleGetDetails(scrum.id)
            } >
            Get Details { " " } <
            /button>{" "} < /
            li >
