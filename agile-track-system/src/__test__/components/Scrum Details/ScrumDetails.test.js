...
describe("boundary", () => {
    ...
    beforeAll(() => {
        const filePath = path.resolve(
            __dirname,
            "../../../components/Scrum Details/ScrumDetails.js"
        );
    ...
    test("ScrumDetailsComponent boundary should check for logged-in user and redirect if not logged in", () => {
        expect(fileContent).toMatch(
            /const checkUser = \(\) => \{[\s\S]*const loggedInUser = JSON.parse\(localStorage.getItem\('user'\)\);[\s\S]*if \(!loggedInUser\) \{[\s\S]*history.push\('\/login'\);/
        );
    });
});